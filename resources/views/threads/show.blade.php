...
<div class="md:w-2/3 w-full mx-auto break-words">
    ...
    <div class="flex justify-center items-center">
        @auth
        <like-button :item="{{ $thread }}" store="{{ route('likes.store', $thread) }}"
            destroy="{{ route('likes.destroy', $thread) }}">
        </like-button>

        <subscribe-button :item="{{ $thread }}" store="{{ route('threads.subscribe', $thread) }}"
        ...
        @can('update', $thread)
        <div
            class="cursor-pointer items-center justify-center rounded-full ml-2 border border-gray-200 inline-block w-12 h-12 flex text-xs text-gray-600 leading-loose hover:text-red-500 hover:border-red-300 focus:outline-none">
