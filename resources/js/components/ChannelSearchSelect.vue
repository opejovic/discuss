        ...
        <div class="relative" :class="{ 'is-active': isOpen }">
            ...
            <transition
                ...
                <div v-if="isOpen" class="bg-gray-200 rounded shadow-lg mt-2 w-64 absolute right-0">
                    <input class="w-full p-3 focus:outline-none bg-gray-200 text-gray-700 select-none rounded-t border-b-2"
                           v-model="search"
                           ref="search"
                           @keydown.esc="close"
                    ...
                    <ul ref="channels" class="mt-2 w-full search-select-options">
                        <li v-for="(channel, i) in filteredChannels"
                            :key="channel.id"
                    ...
                    <div v-show="noResults" class="search-select-empty">No matches for "{{ search }}"</div>
                </div>
            </transition>
        </div>
    </on-click-outside>
    ...
    export default {
        ...
        methods: {
            ...
            highlight(index) {
                ...
                this.$refs.channels.children[this.highlightedIndex].scrollIntoView({block: "nearest"})
            },
            highlightNext() {
